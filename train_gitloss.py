        ...
        with sess.as_default():
            ...
            while epoch < args.num_epochs:
                ...
                train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
                      ...
                      global_step, total_loss, centers_update_op, train_op, summary_op, summary_writer,
...
def train(args, sess, epoch, image_list, label_list, index_dequeue_op, enqueue_op, image_paths_placeholder,
          ...
          loss, centers_update, train_op, summary_op, summary_writer, regularization_losses, saver,
          model_dir, cur_time):
    batch_number = 0
    step = 0
    ...
    while batch_number < args.epoch_size:
        ...
        if batch_number % 100 == 0:
            err, _, step, reg_loss, _, summary_str = sess.run(
                [loss, train_op, global_step, regularization_losses, centers_update, summary_op], feed_dict=feed_dict)
            ...
            err, _, step, reg_loss, _ = sess.run(
                [loss, train_op, global_step, regularization_losses, centers_update], feed_dict=feed_dict)
        duration = time.time() - start_time
        r_loss = float(np.sum(reg_loss))
        print('Epoch: [%d][%d/%d]\tTime %.3f\tLoss %2.3f\tRegLoss %2.3f' %
              (epoch, batch_number + 1, args.epoch_size, duration, err, r_loss))
        batch_number += 1
        train_time += duration
    summary = tf.Summary()
...
def parse_arguments(argv):
    ...
    parser.add_argument('--data_dir', type=str, help='.', default='../faces_emore_aligned_2')
